...
#ifdef _DEBUG
#define DBG_MSG(str) OutputDebugString(str)
#else
#define DBG_MSG(str) 
#endif
...
class Sessao {
    ...
    void SalvarLista() {
        ...
        if (!out) {
            DBG_MSG(L"Erro ao abrir arquivo para salvar lista de dispositivos conhecidos.\n");
		    return;
        }
        ...
        out.close();
		DBG_MSG(L"Lista de dispositivos conhecidos salva com sucesso.\n");
    }
    ...
    void CarregarLista() {
		...
		// verificar se o arquivo foi aberto corretamente
        if (!in) {
            DBG_MSG(L"Erro ao carregar lista de dispositivos conhecidos.\n");
            return;
        }
        ...
        in.close();
		DBG_MSG(L"Lista de dispositivos conhecidos carregada com sucesso.\n");
	}
    ...
    public:
        ...
        bool DispositivoEhConhecido(sockaddr_in clientAddr) {
            ...
            if (it != dispositivosConhecidos.end()) {
                ...
                DBG_MSG(L"Dispositivo conhecido conectado.\n");
                seqId = 0; // resetar seqId para uma nova conexão
                DBG_MSG(L"SeqId resetado.\n");
			    return true; // dispositivo conhecido
            }
            else {
                DBG_MSG(L"Conexão negada, dispositivo desconhecido.\n");
                ...
                if (conexao == 1 || conexao == 0) {
                    if (conexao == 1) {
                        ...
                        if (it == dispositivosConhecidos.end()) {
						    ...
						    DBG_MSG(L"Dispositivo conectado e adicionado à lista de conhecidos.\n");
                        }
                        else {
						    DBG_MSG(L"Dispositivo conectado, mas já conhecido, não adicionado novamente.\n");
                        }
                        seqId = 0; // resetar seqId para uma nova conexão
                        DBG_MSG(L"SeqId resetado.\n");
					    ...
					    ShowWindow(hBtnDesconectar, SW_SHOW); // mostrar o botão "Desconectar"
					    DBG_MSG(L"Dispositivo conectado, mas não confiável.\n");
                        seqId = 0; // resetar seqId para uma nova conexão
                        DBG_MSG(L"SeqId resetado.\n");
				    ...
				    DBG_MSG(L"Usuário recusou a conexão.\n");
                    ocupado = false; // liberar a sessão
				    return AUT_NAO_PERMITIDA; // acesso recusado pelo usuário
                }
            }
            else {
			    DBG_MSG(L"Código incorreto recebido.\n");
                ocupado = false; // liberar a sessão
        ...
        void AtualizarSessao() {
		    ...
		    DBG_MSG(L"Sessão atualizada, esperando nova conexão.\n");
        }
        ...
        void Desconectar() {
            ...
            DBG_MSG(L"Enviando byte de desconexão forçada para cliente conectado.\n");
		    int sent = sendto(socketStatus, (const char*)&desconexao, sizeof(desconexao), 0, (sockaddr*)&clienteAtivo, sizeof(sockaddr_in));
            if (sent == SOCKET_ERROR) {
                DBG_MSG(L"Erro ao enviar byte de desconexão forçada.\n");
            }
            else {
			    DBG_MSG(L"Byte de desconexão forçada enviado com sucesso.\n");
            }
		...
		// método para definir o status do servidor (ocupado ou livre)
        void SetServidorOcupado(bool status) {
            this->ocupado = status;
		}
        ...
        void LimparListaDispositivos(HWND hDlg) {
            if (dispositivosConhecidos.empty()) {
			    DBG_MSG(L"A lista de dispositivos conhecidos já está vazia.\n");
            }
            else {
                ...
                DBG_MSG(L"Lista de dispositivos conhecidos limpa.\n");
            }
        }
        ...
        void AtualizarListBox(HWND hDlg) {
		    ...
		    DBG_MSG(L"ListBox atualizada com os dispositivos conhecidos.\n");
        }
...
int WINAPI WinMain(
	...
	wc.hIcon = (HICON)LoadImage(hInstance, MAKEINTRESOURCE(IDI_ICON1), IMAGE_ICON, 64, 64, LR_DEFAULTCOLOR); // ícone da janela 32x32
	wc.hIconSm = (HICON)LoadImage(hInstance, MAKEINTRESOURCE(IDI_ICON1), IMAGE_ICON, 32, 32, LR_DEFAULTCOLOR); // ícone da janela 16x16
    ...
    if (hWnd == NULL) {
        if (pData) {
            delete pData;
            DBG_MSG(L"Memória pData liberada.\n");
        }
    ...
    try {
        ...
        if (sock1 != INVALID_SOCKET) {
            closesocket(sock1);
            DBG_MSG(L"Socket sock1 fechado.\n");
        }
        if (sock2 != INVALID_SOCKET) {
            closesocket(sock2);
            DBG_MSG(L"Socket sock2 fechado.\n");
        }
        if (sock3 != INVALID_SOCKET) {
            closesocket(sock3);
            DBG_MSG(L"Socket sock3 fechado.\n");
        }
        DestroyWindow(hWnd);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    switch (message) {
	    ...
	    case WM_DESTROY: {// tratar a mensagem WM_DESTROY, dessa vez, usada para liberar e finalizar o aplicativo
            ...
            DBG_MSG(L"Icône da bandeja removido.\n");
...
void VerificarStatus(unsigned short port, SOCKET& outSocket) {
    try {
        ...
        DBG_MSG(L"Servidor UDP escutando na porta: 54321.\n");
        ...
        while (rodando.load()) {
            ...
            if (recvLen == SOCKET_ERROR) {
                ...
                if (erro == WSAEINTR || erro == WSAENOTSOCK) {
					DBG_MSG(L"Servidor encerrado ou erro de conexão.\n");
                    break;
                }
                DBG_MSG(L"Erro ao receber pacote do cliente.\n");
                continue;
            }
            DBG_MSG(L"Pacote recebido.\n");

			// verificar se o byte recebido é de desconexão
            if (buffer[0] == DESCONEXAO && sessaoAtual.DispositivoConectado(clientAddr)) {
				// fechar a conexão com o cliente ativo, se houver
				sessaoAtual.Desconectar(); // desconectar o cliente ativo, se houver
				DBG_MSG(L"Byte de desconexão recebido, cliente desconectado.\n");
				continue; // continuar o loop para esperar novos pacotes
            }
            ...
            if (sessaoAtual.ServidorOcupado()) {
                if (sessaoAtual.DispositivoConectado(clientAddr)) {
                    ...
                    if( sent == SOCKET_ERROR) {
                        DBG_MSG(L"Erro ao enviar resposta de cliente conectado.\n");
						continue;
					}
					DBG_MSG(L"Cliente conectado, resposta enviada com sucesso.\n");
					...
					int sent = sendto(sock, (const char*)&respostaStatus, sizeof(respostaStatus), 0, (sockaddr*)&clientAddr, addrLen);
                    if (sent == SOCKET_ERROR) {
                        DBG_MSG(L"Erro ao enviar resposta de servidor ocupado.\n");
						continue;
                    }
					DBG_MSG(L"Servidor ocupado, resposta enviada com sucesso.\n");
                }
            }
            else {
                ...
                int sent = sendto(sock, (const char*)&respostaStatus, sizeof(respostaStatus), 0, (sockaddr*)&clientAddr, addrLen);
                if (sent == SOCKET_ERROR) {
                    DBG_MSG(L"Erro ao enviar resposta de servidor livre.\n");
					continue;
				}
				DBG_MSG(L"Servidor livre, resposta enviada com sucesso.\n");
            }
        }
    }
...
void ReceberComandos(unsigned short port, SOCKET& outSocketUDP, SOCKET& outSocketTCP, AppData* pData) {
    try {
        ...
        DBG_MSG(L"Servidor escutando na porta UDP: 53100 \nServidor escutando na porta TCP: 53100\n");
        ...
        while (rodando.load()) {
            ...
            if (FD_ISSET(sockTCP, &readfds)) {
                ...
                if (novoCliente == INVALID_SOCKET) {
                    DBG_MSG(L"Erro ao aceitar conexão TCP.\n");
                ...
                if (recvLen == SOCKET_ERROR || recvLen == 0) {
                    ...
                    if (erro == WSAEINTR || erro == WSAENOTSOCK || erro == WSAECONNRESET || erro == WSAETIMEDOUT) {
                        ...
                        DBG_MSG(L"Conexão finalizada ou timeout.\n");
						continue; // continuar o loop para receber mais mensagens
                    }
                    DBG_MSG(L"Erro ao receber dados do cliente.\n");
                    closesocket(novoCliente); // fechar o socket se houver erro
                    novoCliente = INVALID_SOCKET; // evitar uso de socket inválido
					continue; // continuar o loop para receber mais mensagens
                }
                ...
                if (sessaoAtual.ServidorOcupado()) {
                    if(sessaoAtual.DispositivoConectado(clientAddr)) {
                        DBG_MSG(L"Cliente já conectado, enviando resposta para o cliente.\n");
                        respostaConexao = CLIENTE_CONECTADO; // resposta de cliente já conectado
                        // enviar resposta para o cliente
                        int bytesSent = send(novoCliente, (const char*)&respostaConexao, sizeof(respostaConexao), 0); 
                        if (bytesSent == SOCKET_ERROR) {
                            DBG_MSG(L"Erro ao enviar resposta para o cliente.\n");
                        }
                        closesocket(novoCliente); // fechar socket do cliente
                        novoCliente = INVALID_SOCKET; // evitar uso de socket inválido
                        continue; // continuar o loop para receber mais mensagens
					}else {
                        DBG_MSG(L"Servidor ocupado, enviando resposta para o cliente.\n");
				...
				sessaoAtual.SetServidorOcupado(true); // marcar o servidor como ocupado

                int tamanhoMensagem = bufferTCP[0]; // tamanho da mensagem recebida
                if (tamanhoMensagem > tamanhoBufferTCP) {
                    DBG_MSG(L"Tamanho da mensagem recebida é maior que o buffer TCP, enviando resposta para o cliente.\n");
                    ...
                    novoCliente = INVALID_SOCKET; // evitar uso de socket inválido
					sessaoAtual.SetServidorOcupado(false); // marcar o servidor como livre
                ...
                if (offset == tamanhoMensagem) {
                    DBG_MSG(L"Dados recebidos com sucesso do cliente \n");
                    switch (bufferTCP[1]) {
					    case AUTENTICACAO_CODIGO: { // tratar autenticação por código
                            DBG_MSG(L"Autenticação por código recebida \n");
                            ...
                            switch (respostaAutenticacao) {
                                ...
                                case AUT_NAO_PERMITIDA: { // autenticação não permitida
                                    DBG_MSG(L"Autenticação não permitida.\n");
                                    // enviar resposta para o cliente
                                ...
                                case AUT_PERMITIDA: { // autenticação permitida, dispositivo não confiável
                                    ...
                                    DBG_MSG(L"Autenticação permitida, dispositivo não confiável.\n");
                                    break;
                                }
                                case AUT_PERMITIDA_C: { // autenticação permitida, dispositivo confiável
                                    ...
                                    DBG_MSG(L"Autenticação permitida, dispositivo confiável.\n");
                                    break;
                                }
                            }
                            break;
					    ...
					    case AUTENTICACAO_C: { // tratar autenticação por dispositivo confiável
                            ...
                            if (sessaoAtual.DispositivoEhConhecido(clientAddr)) {
                                ...
                                DBG_MSG(L"Dispositivo confiável conectado.\n");
                            }
                            else {
                                ...
                                DBG_MSG(L"Dispositivo desconhecido tentou se conectar.\n");
                            }
                            break;
                        }
                    }
                    recv(novoCliente, (char*)bufferTCP + offset, tamanhoBufferTCP - offset, 0); // esperar o cliente mandar confirmação que recebeu a mensagem
					DBG_MSG(L"Conexão finalizada com sucesso.\n");
                    closesocket(novoCliente); // fechar o socket após receber os dados
                    novoCliente = INVALID_SOCKET; // evitar uso de socket inválido
					continue; // continuar o loop para receber mais mensagens
                }
                else {
                    DBG_MSG(L"Mensagem incompleta.\n");
            ...
            if (FD_ISSET(sockUDP, &readfds)) {
                ...
                if (recvLen == SOCKET_ERROR) {
                    ...
                    DBG_MSG(L"Não foi possível receber pacote do cliente.\n");
                    continue;
                }

				// verificar se o cliente que enviou os comandos está conectado
                if (!sessaoAtual.DispositivoConectado(clientAddr)) {
                    DBG_MSG(L"Dispositivo não conectado, portanto ignorar comandos.\n");
					continue; // ignorar comandos de dispositivos não conectados
                }

                DBG_MSG(L"Comando recebido! \n");
                ...
                if (CheckSum(bufferUDP) && bufferUDP[0] != seqId && uint8_t(bufferUDP[0] - seqId) <= 127) {
                    DBG_MSG(L"Comando válido e em sequência \n");
                    ...
                    DBG_MSG(L"Comando fora de sequência, duplicado ou adulterado.\n");
                }
            }
        }
    }
...
bool CheckSum(const uint8_t(&buffer)[5]) {
    ...
    for (int i = 0; i < 4; ++i) {
        OutputDebugStringA(("Byte[" + std::to_string(static_cast<int>(i)) + "] = " + std::to_string(static_cast<int>(buffer[i])) + "\n").c_str());
    }
...
void SimularMouse(const uint8_t(&buffer)[5]) {// interpretar os dados recebidos e simular o mouse
    ...
    switch (buffer[1]) {
        case MOUSE_MOVE: { // mover mouse
            DBG_MSG(L"Comando de movimento de mouse recebido.\n");
            ...
            if (SendInput(1, input, sizeof(INPUT)) == 0) {
                DBG_MSG(L"Erro ao movimentar mouse.\n");
            }
			DBG_MSG(L"Mouse movido com sucesso.\n");
            break;
        }
        case MOUSE_RIGHTBUTTON: { // pressionar botão direito do mouse
			// verificar se o comando está correto
            if (buffer[3] == 0) {
                DBG_MSG(L"Comando para pressionar botão direito do mouse recebido.\n");
				input[0].type = INPUT_MOUSE; // definir o tipo de entrada como mouse
                input[0].mi.dwFlags = static_cast<DWORD>(buffer[2]); // botão direito
				// emular entrada do mouse
                if (SendInput(1, input, sizeof(INPUT)) == 0) {
                    DBG_MSG(L"Erro ao pressionar botão direito do mouse.\n");
                }
				DBG_MSG(L"Botão direito do mouse pressionado com sucesso.\n");
                break;
            }
            DBG_MSG(L"Comando desconhecido recebido!\n");
            break;
        }
		case MOUSE_LEFTBUTTON: { // pressionar botão esquerdo do mouse
			// verificar se o comando está correto
            if (buffer[3] == 0) {
                DBG_MSG(L"Comando para pressionar botão esquerdo do mouse recebido.\n");
				input[0].type = INPUT_MOUSE; // definir o tipo de entrada como mouse
                input[0].mi.dwFlags = static_cast<DWORD>(buffer[2]); // botão esquerdo
				// emular entrada do mouse
                if (SendInput(1, input, sizeof(INPUT)) == 0) {
                    DBG_MSG(L"Erro ao pressionar botão esquerdo do mouse.\n");
                }
				DBG_MSG(L"Botão esquerdo do mouse pressionado com sucesso.\n");
                break;
            }
            DBG_MSG(L"Comando desconhecido recebido!\n");
            break;
		...
		case MOUSE_MIDDLEBUTTON: { // pressionar botão do meio do mouse
			// verificar se o comando está correto
            if (buffer[3] == 0) {
                DBG_MSG(L"Comando para pressionar botão do meio do mouse recebido.\n");
				input[0].type = INPUT_MOUSE; // definir o tipo de entrada como mouse
                input[0].mi.dwFlags = static_cast<DWORD>(buffer[2]); // botão do meio
				// emular entrada do mouse
                if (SendInput(1, input, sizeof(INPUT)) == 0) {
                    DBG_MSG(L"Erro ao pressionar botão do meio do mouse.\n");
                }
				DBG_MSG(L"Botão do meio do mouse pressionado com sucesso.\n");
                break;
            }
            DBG_MSG(L"Comando desconhecido recebido!\n");
            break;
		...
		case MOUSE_WHEEL: { // rodar a roda do mouse
			// verificar se o comando está correto
            if (buffer[3] == 0) {
                DBG_MSG(L"Comando para rodar a roda do mouse recebido.\n");
				input[0].type = INPUT_MOUSE; // definir o tipo de entrada como mouse
				...
				// emular entrada do mouse
                if (SendInput(1, input, sizeof(INPUT)) == 0) {
                    DBG_MSG(L"Erro ao rodar a roda do mouse.\n");
                }
				DBG_MSG(L"Roda do mouse rodada com sucesso.\n");
                break;
            }
            DBG_MSG(L"Comando desconhecido recebido!\n");
            break;
